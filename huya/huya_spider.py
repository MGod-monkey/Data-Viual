...
Description: 获取虎牙所有的直播间数据(包括直播间名，主播名，直播类型，人气值)，
...
# url = 'https://www.huya.com/cache.php?m=LiveList&do=getLiveListByPage&tagAll=0&page=' + '页数'
...
# 2,获取网页总页数(因为网页中总数并不是固定的，变化幅度较大，直接获取并不准确)
...
try:
    ...
    for num in range(1,400):
        url = 'https://www.huya.com/cache.php?m=LiveList&do=getLiveListByPage&tagAll=0&page=' + str(num)
        print('正在从<' + url + '>获取数据！' + '----当前页:' + str(num))
        try:
            if px.result_code:      # 使用代理
        ...
        if json_data['data']['totalPage'] < num:    # 访问的页数溢出
            break
        else:
            try:
                sudio_name_list = jsonpath(json_data, '$..roomName')  # 直播间名称
                name_list = jsonpath(json_data, '$..nick')    # 主播名
                popularity_list = jsonpath(json_data, '$..totalCount') # 主播人气值
                type_list = jsonpath(json_data, '$..gameFullName')    # 分类
                for i in range(len(sudio_name_list)):
                    data = dict()
                    data['sudio_name'] = sudio_name_list[i]
                    data['name'] = name_list[i]
                    data['popularity'] = int(popularity_list[i])
    ...
    wc.to_file(data_path + '/sudio.png')
    if is_show_wc:  # 展示词云
    ...
    with open(f'{data_path}/error.log', 'a', encoding='utf-8') as fp:
        fp.write(f'[{strftime("%y/%m/%d    %H:%M:%S %a" , localtime(time()))}]\n发生错误：{err}\n')
