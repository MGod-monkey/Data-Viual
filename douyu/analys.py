...
FilePath: /VScode_Worker/Spider/test_wordcould.py
'''
...
'''
:parma 输入数据集，获取相应的数据分析
：datas 数据集
out： 前25位人气主播分析表，数量与人气类型关系表
'''
def analys_of_data(datas):
    ...
    num_list = sorted([num%10000 for num in list(type_dict.values())],reverse=True)     # 获得类型数量的降序列表
    popul_list = sorted([int(num/10000) for num in list(type_dict.values())],reverse=True)     # 获得类型人气值的降序列表
    # 人气前25名的直播内容统计
    ...
    for s in source_popul_3[1:]:
        source.append([s[2],round(s[0]/s[1]/10000,1)])
    source.append([source_popul_other['type'],round(source_popul_other['popularity']/10000/source_popul_other['num'],1)])
    d = (
